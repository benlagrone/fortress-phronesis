...
tar_rc=0
tar --warning=no-file-changed --ignore-failed-read -C "$WP_PATH" -czf "${RELEASE_DIR}/live-wp-content.tar.gz" wp-content || tar_rc=$?
if [ "$tar_rc" -ne 0 ] && [ "$tar_rc" -ne 1 ]; then
  exit "$tar_rc"
fi
if [ "$tar_rc" -eq 1 ]; then
  log 'wp-content changed during read; continuing with created archive'
fi
