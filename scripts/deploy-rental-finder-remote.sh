...
for command_name in curl docker nginx certbot getent; do
...
root_status="$(curl -ksS --resolve "${PUBLIC_HOST}:443:127.0.0.1" -D "${root_headers}" -o /dev/null -w '%{http_code}' "https://${PUBLIC_HOST}/")"
...
api_status="$(curl -ksS --resolve "${PUBLIC_HOST}:443:127.0.0.1" -D "${api_headers}" -o /dev/null -w '%{http_code}' "https://${PUBLIC_HOST}/rental-finder/api/listings/search")"
