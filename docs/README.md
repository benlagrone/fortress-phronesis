# Fortress Phronesis Docs Index (Canonical)

This directory is the authoritative documentation hub for PericopeAI strategy, deployment, and operations.

## Canonical Program Docs

- [Developer Guide](developer-guide.md)
- [Author Acquisition Process](author-acquisition-process.md)
- [Author Acquisition Agents Execution Plan](author-acquisition-agents-execution-plan.md)
- [Author Acquisition Agents Architecture Diagram](author-acquisition-agents-architecture.mmd)
- [Author Work Reader and Passage-Anchored Chat Execution Plan](author-work-reader-passage-chat-execution-plan.md)
- [PericopeAI Deployment Guide](pericopeai-deployment.md)
- [Fortress Personal Deployment](fortress-personal-deployment.md)
- [GitHub Image Control Plane Design](github-image-control-plane-design.md)
- [Deployment Prep Checklist](deployment-prep-checklist.md)
- [Prod Release Runbook v1.1.2](release-runbook-prod-v1.1.2.md)
- [Prod Release Runbook v1.3.1 Phase A](release-runbook-prod-v1.3.1-phase-a.md)
- [Release Commitment v1.1.3 (All Tracks)](release-commitment-v1.1.3-all-tracks.md)
- [Acceptance Test + Demo Guide v1.1.3](acceptance-demo-guide-v1.1.3.md)
- [Reference Inference Test Cases v1.1.3](reference-inference-test-cases-v1.1.3.md)
- [Citation, Cross-Reference, and Author Discovery Agent Plan](citation-cross-reference-author-discovery-plan.md)
- [Reference Intelligence Agents Implementation Plan](reference-intelligence-agents-implementation-plan.md)
- [Reference Intelligence Agents Architecture](reference-intelligence-agents-architecture.mmd)
- [Author Historical Context Capability Execution Plan](author-historical-context-capability-execution-plan.md)
- [Author Historical Context Capability Architecture](author-historical-context-capability-architecture.mmd)
- [Author Work Reader and Passage-Anchored Chat Architecture](author-work-reader-passage-chat-architecture.mmd)
- [Prod Release Runbook v1.1.1](release-runbook-prod-v1.1.1.md)
- [Release Runbook v1.1.0](release-runbook-v1.1.0.md)
- [Local Pericope Stack Runbook](local-pericope-stack-runbook.md)
- [Server Environments](server-environments.md)
- [Container Runbook (Dev Server)](dev-server-container-runbook.md)
- [Workspace DS Broker Deployment Contract](workspace-ds-broker-deployment.md)
- [Roadmap](roadmap.md)
- [Graph Conversation Memory Migration](graph-conversation-memory-migration.md)
- [Unpublished Work Audit 2026-06-22](unpublished-work-audit-2026-06-22.md)
- [Author Taxonomy + Browse Contract](author-taxonomy-browse-contract.md)
- [Testing Guide](testing.md)
- [Epic: Identity, Authentication, and Customization](epic-identity-auth-customization.md)
- [Testing + Hardening Plan (Identity/Auth/Customization)](identity-auth-customization-testing-hardening.md)
- [Roadmap Recovery Ledger](roadmap-recovery-ledger.md)
- [Developer RAG Notes](dev_rag.md)
- [System Handbook](system-handbook.md)

## Subproject Doc Indexes

- [AugustineFE Docs](../../AugustineFE/docs/README.md)
- [AugustineService Docs](../../AugustineService/docs/index.md)
- [Model Discernment Engine Docs](../../Model_Discernment_Engine/docs/README.md)
- [Solomonic Seals Docs](../../Solomonic_Seals/docs/README.md)
- [Latin RAG Translator Docs](../../latin-rag-translator/docs/README.md)

- [X/Twitter Persona Control Fortress LAN Surface](twitter-cleanup-control.md)

## Documentation Rules

- Keep existing historical guides; add updates as additive sections or new docs.
- Put cross-project strategy and deployment policy changes in this directory first.
- In subprojects, keep implementation/runbook detail local and link back to this index.
- If two docs conflict, do not silently replace either; raise the conflict and resolve explicitly.
