...
| MEM-002 | Graph conversation memory migration: graph-shaped matter/counsel/session memory, graph DB proof, dual-write/read-shadowing, bounded graph-selected prompt context, and private graph boundary | v1.2.3 | Planned | [graph-conversation-memory-migration.md](graph-conversation-memory-migration.md) |
